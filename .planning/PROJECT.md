...
- SM-01 validated in Phase 4.1.1 — the canonical CodeWiki skill sources now live under `src/templates/skills/codewiki-<name>/SKILL.md` with preserved prompt bodies and skill discovery frontmatter.
...
- BUILD-03..BUILD-04 validated in Phase 8 — package metadata declares `node >=20.11.0`, runtime dependencies remain zero, and local packed `npx` smoke/pack manifest coverage verify the publish candidate.
...
- **Hook formats:** Claude Code uses `.claude/settings.json` with `PreToolUse` and `PostToolUse` on `Write|Edit`. Codex uses `.codex/hooks.json` plus `.codex/config.toml` feature enablement; current Codex docs support `PreToolUse`/`PostToolUse` for `apply_patch` with `Edit|Write` aliases, but Codex requires event-specific stdout handling, so `PostToolUse` and `Stop` need wrappers. Copilot uses `.github/hooks/*.json` with `preToolUse` and `postToolUse`, while `agentStop` is the meaningful post-turn hook and `sessionEnd` is cleanup-only. OpenCode uses `.opencode/plugins/codewiki.ts` and plugin events (`tool.execute.before`, `file.edited`, `session.idle`) instead of `opencode.json` hook wiring.
...
---
*Last updated: 2026-05-03 after adding the Obsidian vault skill and reinstall update fix*
