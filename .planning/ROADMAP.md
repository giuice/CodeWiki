...
- [ ] **Phase 6: OpenCode Adapter** - Skills install to .agents/skills/, agents, and TS plugin hook dispatcher
...
**Success Criteria** (what must be TRUE):
  1. Running `npx codewiki init` in a project with `.claude/` present creates all wiki directories, installs 8 Skills to `.claude/skills/codewiki-<name>/SKILL.md`, installs 2 agents to `.claude/agents/`, and prints a structured install report
...
**Success Criteria** (what must be TRUE):
  ...
  6. REQUIREMENTS.md CC-01 references Skills layout; ROADMAP success criteria reference Skills
**Plans:** 3 plans
Plans:
- [ ] 04.1-01-PLAN.md — Create 8 skill template directories with SKILL.md files, migrate unit tests, delete old command templates
- [ ] 04.1-02-PLAN.md — Rewrite ClaudeCodeAdapter for skills layout, update integration tests and pack test
- [ ] 04.1-03-PLAN.md — Update REQUIREMENTS, ROADMAP, PROJECT, research docs, README, Phase 6 plan; delete stale handoff docs
...
**Success Criteria** (what must be TRUE):
  ...
  3. A test runs `npm pack --dry-run` and asserts `dist/templates/skills/codewiki-ingest/SKILL.md` appears in the file list
...
**Goal**: `npx codewiki init` on a project with `opencode.json` installs 8 Skills to `.agents/skills/codewiki-<name>/SKILL.md`, agents, and a TS plugin hook dispatcher
**Depends on**: Phase 5
**Requirements**: OC-01, OC-02, OC-03, OC-04
**Success Criteria** (what must be TRUE):
  1. Running `npx codewiki init` on a project with `opencode.json` installs 8 Skills to `.agents/skills/codewiki-<name>/SKILL.md` and 2 agents to `.opencode/agents/`
  2. The resulting `opencode.json` contains a hook config pointing to `.codewiki/hooks/` and no PreToolUse entry
...
**Plans:** 2 plans
Plans:
- [ ] 06-01-PLAN.md — Create OpenCode skill, agent, and AGENTS.md template assets
...
**Goal**: Codex and Copilot adapters install Skills to `.agents/skills/` and merge hook configs after per-tool research spikes confirm paths
**Depends on**: Phase 6
...
**Success Criteria** (what must be TRUE):
  ...
  3. Skills are installed to `.agents/skills/codewiki-<name>/SKILL.md` for Codex and Copilot (plus `.claude/skills/` when Claude Code is also selected)
  4. If Copilot has no confirmed skill directory beyond `.agents/skills/`, the install report documents the limitation rather than silently skipping
**Plans:** TBD
Plans:
- [ ] TBD

### Phase 8: npm Publish Hardening
**Goal**: The package publishes to npm correctly and `npx codewiki init` works in a fresh project with all skill files present in the tarball
**Depends on**: Phase 7
...
| 4.1 Skills Migration (INSERTED) | 0/3 | Not started | - |
