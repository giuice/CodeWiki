...
export async function statusCommand(_args: string[], root = process.cwd()): Promise<string> {
  ...
  for (const page of pages) {
    ...
    if (frontmatterString(parsed.frontmatter.type) === "issue") {
      if (frontmatterString(parsed.frontmatter.status) === "resolved") resolvedIssues += 1;
      else openIssues += 1;
    }
  }
  const logText = (await readTextIfExists(root, `${config.wiki.path.replace(/\/$/, "")}/log.md`)) ?? "";
  const latestLog = (logText.split(/\r?\n/).filter((line) => line.startsWith("## ")).at(-1)?.replace(/^##\s+/, "") ?? firstHeading(logText) ?? "No log heading found");
