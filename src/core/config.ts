...
import { SUPPORTED_TOOLS, type CodeWikiConfig, type SupportedTool } from "./types.js";
...
function parseNumber(value: string, key: string): number {
  ...
  if (!Number.isFinite(parsed)) throw new Error(`Unsupported numeric value for ${key}: ${value}`);
  return parsed;
}
...
function assertSupportedYamlSubset(text: string): void {
...
export function parseGeneratedConfig(text: string, root = process.cwd()): CodeWikiConfig {
  ...
  if (config.tools.length === 0) config.tools = [...DEFAULT_CONFIG.tools];
  ensureWithinRoot(root, config.wiki.path);
  ensureWithinRoot(root, config.wiki.rawPath);
  return config;
}

export async function loadConfig(root = process.cwd()): Promise<CodeWikiConfig> {
  ...
  if (text === undefined) throw new Error("Unable to read .codewiki/config.yml");
