...
- `UserPromptSubmit` may load short prompt-level wiki context from `.codewiki/hooks/pre-wiki-context.mjs` when the prompt is wiki-relevant
...
- `PreToolUse` is not wired by default; `.codex/hooks/pre-tool-use.mjs` remains packaged for future opt-in blocking guardrails
- `PostToolUse` records deduped pending absorb state through `.codewiki/hooks/post-verify.mjs`; visible hook context is debug-only
- `Stop` wraps `.codewiki/hooks/session-end.mjs` without blocking by default, records deduped lifecycle state, and respects `stop_hook_active` to avoid continuation loops
