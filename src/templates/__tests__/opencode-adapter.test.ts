...
describe("OC-03: OpenCode plugin template stays a thin shared-hook dispatcher", () => {
  test("plugin template contains the documented events and shared hook targets", async () => {
    ...
    expect(content).toContain("pre-wiki-context.mjs");
    expect(content).toContain("post-verify.mjs");
    expect(content).toContain("session-end.mjs");
    expect(content).toContain("codewikiContext");
    expect(content).toContain("spawn(process.execPath");
    ...
    expect(content).toContain("HOOK_TIMEOUT_MS = 30000");
