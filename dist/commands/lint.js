...
function knownPageKeys(page, id) {
...
export async function collectLintFindings(root = process.cwd()) {
    ...
    for (const page of pages) {
        ...
        for (const key of knownPageKeys(page, frontmatterString(parsed.frontmatter.id)))
            known.add(key);
        incoming.set(page, 0);
    }
...
export async function lintCommand(_args, root = process.cwd()) {
