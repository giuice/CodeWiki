...
function markdownId(filePath, content) {
...
export async function collectLintFindings(cwd) {
    const findings = [];
    ...
    try {
        ...
        findings.push({ severity: "error", category: "required-file", path: ".codewiki/config.yml", message: error.message });
    }
    ...
    for (const page of pages) {
        ...
        for (const id of markdownId(page, content))
            knownIds.add(id);
    }
    ...
    for (const [page, content] of pageContents) {
        ...
        if ((page.includes("/entities/") || data.type === "entity") && data.file_hashes && typeof data.file_hashes === "object" && !Array.isArray(data.file_hashes)) {
            for (const [relativeFile, expected] of Object.entries(data.file_hashes)) {
