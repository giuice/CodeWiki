import { mkdir, readFile, readdir, stat, writeFile } from "node:fs/promises";
import { createHash } from "node:crypto";
import path from "node:path";
export function ensureInsideRoot(root, candidate) {
    const resolvedRoot = path.resolve(root);
    const resolved = path.resolve(root, candidate);
    if (resolved !== resolvedRoot && !resolved.startsWith(`${resolvedRoot}${path.sep}`)) {
        throw new Error(`Path escapes project root: ${candidate}`);
    }
    return resolved;
}
export function relativePath(root, absolutePath) {
    return path.relative(path.resolve(root), path.resolve(absolutePath)).split(path.sep).join("/");
}
export async function exists(filePath) {
    try {
        await stat(filePath);
        return true;
    }
    catch (error) {
        if (error.code === "ENOENT")
            return false;
        throw error;
    }
}
export async function readText(filePath) {
    return readFile(filePath, "utf8");
}
export async function readTextIfExists(filePath) {
    if (!(await exists(filePath)))
        return undefined;
    return readText(filePath);
}
export async function writeFileSafe(root, relativeFile, content, force) {
    const target = ensureInsideRoot(root, relativeFile);
    await mkdir(path.dirname(target), { recursive: true });
    if (!force && (await exists(target))) {
        const current = await stat(target);
        if (current.size > 0) {
            throw new Error(`Refusing to overwrite existing non-empty file without --force: ${relativeFile}`);
        }
    }
    await writeFile(target, content, "utf8");
}
export async function ensureDir(root, relativeDir) {
    await mkdir(ensureInsideRoot(root, relativeDir), { recursive: true });
}
export function isMarkdownPath(filePath) {
    const lower = filePath.toLowerCase();
    return lower.endsWith(".md") || lower.endsWith(".markdown");
}
export async function listMarkdownFiles(root, relativeDir) {
    const base = ensureInsideRoot(root, relativeDir);
    if (!(await exists(base)))
        return [];
    const found = [];
    async function walk(current) {
        const entries = await readdir(current, { withFileTypes: true });
        for (const entry of entries) {
            const full = path.join(current, entry.name);
            if (entry.isDirectory()) {
                await walk(full);
            }
            else if (entry.isFile() && isMarkdownPath(entry.name)) {
                found.push(relativePath(root, full));
            }
        }
    }
    await walk(base);
    return found.sort();
}
export async function sha256File(filePath) {
    const data = await readFile(filePath);
    return createHash("sha256").update(data).digest("hex");
}
export function slugify(input) {
    const slug = input
        .toLowerCase()
        .replace(/[`'"“”‘’]/g, "")
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "")
        .slice(0, 72);
    return slug || "untitled";
}
export function timestampForFile(date = new Date()) {
    return date.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
}
//# sourceMappingURL=files.js.map