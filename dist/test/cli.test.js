...
function runCodeWiki(args, cwd = repoRoot) {
...
async function listFiles(root, relDir) {
    const out = new Map();
    async function visit(absDir) {
        ...
        for (const entry of await readdir(absDir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                ...
                out.set(path.relative(root, abs).split(path.sep).join("/"), await readFile(abs, "utf8"));
        }
    }
...
test("package baseline and CLI help match the TypeScript npm contract", async () => {
    ...
    const tsconfig = JSON.parse(await readFile(path.join(repoRoot, "tsconfig.json"), "utf8"));
